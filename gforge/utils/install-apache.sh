...
case "$1" in
	...
	if [ -f /usr/sbin/a2enmod ] ; then
		for flavour in apache2 ;  do
			if [ -e /etc/$flavour/httpd.conf ] ; then
				DEBIAN_FRONTEND=noninteractive /usr/sbin/a2enmod php4
				DEBIAN_FRONTEND=noninteractive /usr/sbin/a2enmod ssl
				DEBIAN_FRONTEND=noninteractive /usr/sbin/a2enmod suexec
				#not enabling env module, part of base in apache2
				DEBIAN_FRONTEND=noninteractive /usr/sbin/a2enmod vhost_alias
