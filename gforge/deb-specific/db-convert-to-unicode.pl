...
eval {
    ...
    convert_column_to_charset ('db_images', 'response_text', $from, $to, -1) ;
    ...
    convert_column_to_charset ('doc_groups', 'filetype', $from, $to, -1) ;
    ...
    convert_column_to_charset ('forum_agg_msg_count', 'body', $from, $to, -1) ;

    convert_column_to_charset ('forum_group_list', 'body', $from, $to, -1) ;
    ...
    convert_column_to_charset ('forum_monitored_forums', 'send_all_posts_to', $from, $to, -1) ;

    convert_column_to_charset ('forum_saved_place', 'send_all_posts_to', $from, $to, -1) ;

    convert_column_to_charset ('frs_file', 'send_all_posts_to', $from, $to, -1) ;
    convert_column_to_charset ('frs_file', 'filename', $from, $to, -1) ;

    convert_column_to_charset ('frs_filetype', 'filename', $from, $to, -1) ;
    ...
    convert_column_to_charset ('groups', 'dead4', $from, $to, -1) ;
    convert_column_to_charset ('groups', 'dead5', $from, $to, -1) ;
    convert_column_to_charset ('groups', 'dead6', $from, $to, -1) ;
    convert_column_to_charset ('groups', 'new_doc_address', $from, $to, -1) ;

    convert_column_to_charset ('mail_group_list', 'new_doc_address', $from, $to, -1) ;
    ...
    convert_column_to_charset ('project_history', 'send_all_posts_to', $from, $to, -1) ;
    ...
    convert_column_to_charset ('project_metric', 'old_value', $from, $to, -1) ;

    convert_column_to_charset ('project_metric_tmp1', 'old_value', $from, $to, -1) ;

    convert_column_to_charset ('project_status', 'old_value', $from, $to, -1) ;
    convert_column_to_charset ('project_status', 'status_name', $from, $to, -1) ;

    convert_column_to_charset ('project_task', 'status_name', $from, $to, -1) ;
    convert_column_to_charset ('project_task', 'summary', $from, $to, -1) ;
    convert_column_to_charset ('project_task', 'details', $from, $to, -1) ;

    convert_column_to_charset ('project_weekly_metric', 'details', $from, $to, -1) ;

    convert_column_to_charset ('session', 'details', $from, $to, -1) ;

    convert_column_to_charset ('snippet', 'details', $from, $to, -1) ;
    ...
    convert_column_to_charset ('snippet_package', 'license', $from, $to, -1) ;
    convert_column_to_charset ('snippet_package', 'name', $from, $to, -1) ;
    convert_column_to_charset ('snippet_package', 'description', $from, $to, -1) ;

    convert_column_to_charset ('snippet_package_item', 'description', $from, $to, -1) ;

    convert_column_to_charset ('snippet_package_version', 'description', $from, $to, -1) ;
    ...
    convert_column_to_charset ('stats_agg_logo_by_day', 'code', $from, $to, -1) ;

    convert_column_to_charset ('stats_agg_pages_by_day', 'code', $from, $to, -1) ;

    convert_column_to_charset ('survey_question_types', 'code', $from, $to, -1) ;
    convert_column_to_charset ('survey_question_types', 'type', $from, $to, -1) ;

    convert_column_to_charset ('survey_questions', 'type', $from, $to, -1) ;
    convert_column_to_charset ('survey_questions', 'question', $from, $to, -1) ;

    convert_column_to_charset ('survey_rating_aggregate', 'question', $from, $to, -1) ;

    convert_column_to_charset ('survey_rating_response', 'question', $from, $to, -1) ;

    convert_column_to_charset ('survey_responses', 'question', $from, $to, -1) ;
    convert_column_to_charset ('survey_responses', 'response', $from, $to, -1) ;

    convert_column_to_charset ('surveys', 'response', $from, $to, -1) ;
    convert_column_to_charset ('surveys', 'survey_title', $from, $to, -1) ;
    convert_column_to_charset ('surveys', 'survey_questions', $from, $to, -1) ;

    convert_column_to_charset ('trove_cat', 'survey_questions', $from, $to, -1) ;
    ...
    convert_column_to_charset ('trove_group_link', 'fullpath_ids', $from, $to, -1) ;

    convert_column_to_charset ('user_bookmarks', 'fullpath_ids', $from, $to, -1) ;
    ...
    convert_column_to_charset ('user_diary', 'bookmark_title', $from, $to, -1) ;
    convert_column_to_charset ('user_diary', 'summary', $from, $to, -1) ;
    convert_column_to_charset ('user_diary', 'details', $from, $to, -1) ;

    convert_column_to_charset ('user_diary_monitor', 'details', $from, $to, -1) ;

    convert_column_to_charset ('user_group', 'details', $from, $to, -1) ;

    convert_column_to_charset ('user_metric', 'details', $from, $to, -1) ;

    convert_column_to_charset ('user_metric0', 'details', $from, $to, -1) ;

    convert_column_to_charset ('user_preferences', 'details', $from, $to, -1) ;
    ...
    convert_column_to_charset ('user_ratings', 'preference_value', $from, $to, -1) ;

    convert_column_to_charset ('users', 'preference_value', $from, $to, -1) ;
    ...
    convert_column_to_charset ('project_sums_agg', 'jabber_address', $from, $to, -1) ;

    convert_column_to_charset ('prdb_dbs', 'jabber_address', $from, $to, -1) ;
    ...
    convert_column_to_charset ('artifact_group_list', 'cgidir', $from, $to, -1) ;
    ...
    convert_column_to_charset ('artifact_resolution', 'browse_instructions', $from, $to, -1) ;
    convert_column_to_charset ('artifact_resolution', 'resolution_name', $from, $to, -1) ;

    convert_column_to_charset ('artifact_perm', 'resolution_name', $from, $to, -1) ;

    convert_column_to_charset ('artifact_category', 'resolution_name', $from, $to, -1) ;
    convert_column_to_charset ('artifact_category', 'category_name', $from, $to, -1) ;

    convert_column_to_charset ('artifact_group', 'category_name', $from, $to, -1) ;
    convert_column_to_charset ('artifact_group', 'group_name', $from, $to, -1) ;

    convert_column_to_charset ('artifact_status', 'group_name', $from, $to, -1) ;
    convert_column_to_charset ('artifact_status', 'status_name', $from, $to, -1) ;

    convert_column_to_charset ('artifact', 'status_name', $from, $to, -1) ;
    convert_column_to_charset ('artifact', 'summary', $from, $to, -1) ;
    convert_column_to_charset ('artifact', 'details', $from, $to, -1) ;

    convert_column_to_charset ('artifact_history', 'details', $from, $to, -1) ;
    ...
    convert_column_to_charset ('artifact_file', 'old_value', $from, $to, -1) ;
    ...
    convert_column_to_charset ('artifact_message', 'filetype', $from, $to, -1) ;
    convert_column_to_charset ('artifact_message', 'from_email', $from, $to, -1) ;
    convert_column_to_charset ('artifact_message', 'body', $from, $to, -1) ;

    convert_column_to_charset ('artifact_monitor', 'body', $from, $to, -1) ;
    convert_column_to_charset ('artifact_monitor', 'email', $from, $to, -1) ;

    convert_column_to_charset ('artifact_canned_responses', 'email', $from, $to, -1) ;
    ...
    convert_column_to_charset ('artifact_counts_agg', 'body', $from, $to, -1) ;

    convert_column_to_charset ('stats_site_pages_by_day', 'body', $from, $to, -1) ;

    convert_column_to_charset ('massmail_queue', 'body', $from, $to, -1) ;
    ...
    convert_column_to_charset ('stats_agg_site_by_group', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_project_metric', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_agg_logo_by_group', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_subd_pages', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_cvs_user', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_cvs_group', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_project_developers', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_project', 'message', $from, $to, -1) ;

    convert_column_to_charset ('stats_site', 'message', $from, $to, -1) ;

    convert_column_to_charset ('activity_log_old_old', 'message', $from, $to, -1) ;
    ...
    convert_column_to_charset ('user_metric_history', 'page', $from, $to, -1) ;

    convert_column_to_charset ('frs_dlstats_filetotal_agg', 'page', $from, $to, -1) ;

    convert_column_to_charset ('stats_project_months', 'page', $from, $to, -1) ;

    convert_column_to_charset ('stats_site_pages_by_month', 'page', $from, $to, -1) ;

    convert_column_to_charset ('stats_site_months', 'page', $from, $to, -1) ;

    convert_column_to_charset ('trove_agg', 'page', $from, $to, -1) ;
    ...
    convert_column_to_charset ('group_cvs_history', 'ip_address', $from, $to, -1) ;
    ...
    convert_column_to_charset ('skills_data_types', 'classname', $from, $to, -1) ;
    ...
    convert_column_to_charset ('project_task_artifact', 'category_name', $from, $to, -1) ;

    convert_column_to_charset ('project_group_forum', 'category_name', $from, $to, -1) ;

    convert_column_to_charset ('project_group_doccat', 'category_name', $from, $to, -1) ;

    convert_column_to_charset ('project_messages', 'category_name', $from, $to, -1) ;
    convert_column_to_charset ('project_messages', 'body', $from, $to, -1) ;

    convert_column_to_charset ('plugins', 'body', $from, $to, -1) ;
    convert_column_to_charset ('plugins', 'plugin_name', $from, $to, 32) ;
    convert_column_to_charset ('plugins', 'plugin_desc', $from, $to, -1) ;

    convert_column_to_charset ('group_plugin', 'plugin_desc', $from, $to, -1) ;

    convert_column_to_charset ('user_plugin', 'plugin_desc', $from, $to, -1) ;
