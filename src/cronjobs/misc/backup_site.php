...
if (!is_dir($sys_path_to_backup)) {
	if (!mkdir($sys_path_to_backup, 0700, true)) {
		cron_entry(23, 'Could not create directory '.$sys_path_to_backup.' for backups');
		exit;
	}
}
...
if (file_exists(forge_get_config('upload_dir'))) {
	...
	$err.= _('Unable to find upload dir. Configured value is')._(': ').forge_get_config('upload_dir');
}
...
if (file_exists($mailman_data_dir)) {
	...
	$err.= _('Unable to find Mailman data dir. Please edit backup_site.php cronjob');
}
...
if (file_exists(forge_get_config('repos_path', 'scmcvs'))) {
	...
	$err.= _('Unable to find CVSROOT dir. Configured value is')._(': ').forge_get_config('repos_path', 'scmcvs');
}
...
if (file_exists($svndir_prefix)) {
	...
	$err.= _('Unable to find SVNROOT dir. Configured value is')._(': ').$svndir_prefix;
}
