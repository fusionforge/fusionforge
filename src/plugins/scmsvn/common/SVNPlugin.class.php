...
some control over it to the project's administrator.");
	...
	function topModule($project) {
		$scm_paths = array();
		$scm_paths_file = forge_get_config('groupdir_prefix').'/'.$project->getUnixName().'/'.$project->getUnixName().'_scmsvn_paths.txt';
		...
		foreach ($modules as $module) {
			if (forge_get_config('use_ssh', 'scmsvn')) {
				...
				if (forge_get_config('use_shell_limited')) {
					$b .= '<span class="tt">svn '.$ssh_port.' checkout svn://'.$this->getBoxForProject($project).'/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
				} else {
					$b .= '<span class="tt">svn '.$ssh_port.' checkout svn://'.$this->getBoxForProject($project).$this->svn_root_fs.'/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
				}
			}
			if (forge_get_config('use_dav', 'scmsvn')) {
				$b .= '<span class="tt">svn checkout http'.((forge_get_config('use_ssl', 'scmsvn')) ? 's' : '').'://'.$this->getBoxForProject($project).'/anonscm/svn/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
			}
		}
		$b .= '</p>';
		return $b;
	...
	function getInstructionsForRW($project) {
		...
		if (session_loggedin()) {
			...
			if (forge_get_config('use_ssh', 'scmsvn')) {
				...
				foreach ($modules as $module) {
					if (forge_get_config('use_shell_limited')) {
						$b .= '<span class="tt">svn '.$ssh_port.'checkout svn+ssh://'.$d.'@'.$this->getBoxForProject($project).'/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
					} else {
						$b .= '<span class="tt">svn '.$ssh_port.'checkout svn+ssh://'.$d.'@'.$this->getBoxForProject($project).$this->svn_root_fs .'/'. $project->getUnixName().'/'.$module.'</span>'.html_e('br');
					}
				}
				$b .= '</p></div>';
			}
			if (forge_get_config('use_dav', 'scmsvn')) {
				...
				$b .= '</p><p>';
				foreach ($modules as $module) {
					$b .= '<span class="tt">svn checkout --username '.$d.' http'.((forge_get_config('use_ssl', 'scmsvn')) ? 's' : '').'://'.$this->getBoxForProject($project).'/authscm/'.$d.'/svn/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
				}
				$b .= '</p></div>';
			}
		} else {
			if (forge_get_config('use_ssh', 'scmsvn')) {
				...
				foreach ($modules as $module) {
					if (forge_get_config('use_shell_limited')) {
						$b .= '<span class="tt">svn '.$ssh_port.'checkout svn+ssh://<i>'._('developername').'</i>@'.$this->getBoxForProject($project).'/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
					} else {
						$b .= '<span class="tt">svn '.$ssh_port.'checkout svn+ssh://<i>'._('developername').'</i>@'.$this->getBoxForProject($project).$this->svn_root_fs .'/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
					}
				}
				$b .= '</p></div>';
			}
			if (forge_get_config('use_dav', 'scmsvn')) {
				...
				$b .= '</p><p>';
				foreach ($modules as $module) {
					$b .= '<span class="tt">svn checkout --username <i>'._('developername').'</i> http'.((forge_get_config('use_ssl', 'scmsvn')) ? 's' : '').'://'.$this->getBoxForProject($project).'/authscm/<i>'._('developername').'</i>/svn/'.$project->getUnixName().'/'.$module.'</span>'.html_e('br');
				}
				$b .= '</p></div>';
			}
		}
