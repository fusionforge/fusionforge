...
echo '<form action="/plugins/'.$projectsHierarchy->name.'?type=group&pluginname='.$projectsHierarchy->name.'&action=addChild&id='.$group_id.'" method="post" name="formson">';
//select box of sons
echo '<table><tr>';
echo '<td>'._('Select a project:').'</td><td>'.$projectsHierarchy->son_box($group_id,'sub_project_id','0').'</td><td>&nbsp;</td>' ;
echo '</tr><tr>' ;
echo '<td>'._('Commentary:').'</td><td> <input type="text" size="25" value="" name="com"></td>' ;
//echo type_son_box();
echo '<td><input type="submit" name="son" value="'._('Add Child project').'"></td></tr></table></form>';

//select all the sons of the current project
$res_son = db_query_params('SELECT groups.group_id,
				groups.group_name,
				groups.unix_group_name,
				plugin_projects_hierarchy_relationship.sub_project_id,
				plugin_projects_hierarchy_relationship.status
				FROM groups, plugin_projects_hierarchy_relationship
				WHERE groups.group_id = plugin_projects_hierarchy_relationship.sub_project_id
				AND plugin_projects_hierarchy_relationship.project_id = $1',
			array ($group_id));
