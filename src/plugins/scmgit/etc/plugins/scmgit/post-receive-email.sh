...
# In FusionForge, this is /etc/gforge/plugins/scmgit/post-receive-email.sh:
#
...
keys=domain_name:lists_host:sys_name
...
ff_group=$(<$parentdir/ffgroup.nfo)
...
generate_email_header()
{
	#XXX the content-type may be wrong but it usually helps
	#XXX cf scmsvn commit-email.pl for discussion on this
	...
	Reply-To: $ff_replyto
	From: $ff_sender
	Subject: {$ff_subrepo} $refname_type $short_refname ${change_type}d. $describe
...
send_mail()
{
	/usr/sbin/sendmail -t -f "$ff_sender"
}
