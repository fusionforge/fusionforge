...
//			$b .= $HTML->boxMiddle(_('Repository Statistics'));
	...
	static function createUserRepo($params) {
		...
		if (!is_file("$repodir/HEAD") && !is_dir("$repodir/objects") && !is_dir("$repodir/refs")) {
			...
			system("cd $root;LANG=C git clone --bare --quiet --no-hardlinks $main_repo $repodir 2>&1 >/dev/null | grep -v 'warning: You appear to have cloned an empty repository.' >&2");
	...
	function createOrUpdateRepo($params) {
		...
		for ($i=0; $i<$rows; $i++) {
			...
			if (!is_file("$repodir/HEAD") && !is_dir("$repodir/objects") && !is_dir("$repodir/refs")) {
				if ($clone_url != '') {
					system("cd $root;LANG=C git clone --quiet --bare $clone_url $repodir 2>&1 >/dev/null | grep -v 'warning: You appear to have cloned an empty repository.' >&2");
				} else {
