	...
	function getContent() {
		...
		if (count($GitRepositories)) {
			...
			foreach ($GitRepositories as $GitRepository) {
				...
				$returnhtml .= '<tr><td><tt>git clone git+ssh://'.$user->getUnixName().'@' . $scmgitplugin->getBoxForProject($project) .'/'. forge_get_config('scm_root', 'scmgit') .'/'. $project->getUnixName() .'/users/'. $user->getUnixName() .'.git</tt> ['.util_make_link('/scm/browser.php?group_id='.$project->getID().'&user_id='.$user->getID(), _('Browse')).']</p></td><tr>';
			}
			$returnhtml .= '</table>';
			return $returnhtml;
		} else {
