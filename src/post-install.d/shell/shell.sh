...
#----------------- NSS queries
getpwnam        = SELECT login AS username,'-',gecos,('$homedir_prefix' || login) AS homedir,shell,uid,$gid FROM nss_passwd WHERE login = \$1
getpwuid        = SELECT login AS username,'-',gecos,('$homedir_prefix' || login) AS homedir,shell,uid,$gid FROM nss_passwd WHERE uid = \$1
