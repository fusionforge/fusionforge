CREATE TABLE doc_data_words_idx ( docid integer, group_id integer, vectors tsvector );
CREATE INDEX doc_data_words_idxfti ON doc_data_idx USING gist (vectors);
CREATE TYPE doc_data_words_results AS ( docid integer, title text, description text, data_words text, groupname character varying(255));
...
BEGIN
	...
	ELSIF table_name = ''doc_data'' THEN
		IF TG_OP = ''INSERT'' THEN
			...
			INSERT INTO doc_data_words_idx (docid, group_id, vectors) VALUES (NEW.docid, NEW.group_id, to_tsvector(coalesce(NEW.title,'''') ||'' ''|| coalesce(NEW.description,'''') ||'' ''|| coalesce(NEW.filename,'''') ||'' ''|| coalesce(NEW.data_words,'''')));
		ELSIF TG_OP = ''UPDATE'' THEN
			...
			UPDATE doc_data_idx SET group_id=NEW.group_id, vectors=to_tsvector(coalesce(NEW.title,'''') ||'' ''|| coalesce(NEW.description,'''') ||'' ''|| coalesce(NEW.filename,'''') ||'' ''|| coalesce(NEW.data_words,'''')) WHERE docid=NEW.docid;
...
BEGIN
    FOR r IN select doc_data.docid as docid, doc_data.filename as filename , doc_data.group_id as groupid, doc_data.data_words as datawords, doc_groups.groupname as groupname, doc_data.description as description, doc_data.title as title from doc_data, doc_groups where doc_data.doc_group = doc_groups.doc_group LOOP
       INSERT into doc_data_words_idx (docid, group_id, vectors) values (r.docid, r.groupid, to_tsvector(coalesce(r.title,'''') ||'' ''|| coalesce(r.description,'''') ||'' ''|| coalesce(r.filename,'''') ||'' ''|| coalesce(r.datawords,'''')));
    END LOOP;
	return 1;
END;
' LANGUAGE plpgsql;
