...
BEGIN
	...
	ELSIF table_name = ''doc_data_version'' THEN
		IF TG_OP = ''INSERT'' THEN
			...
			INSERT INTO doc_data_words_idx (docid, version, group_id, vectors) VALUES (NEW.docid, NEW.version, (select group_id from doc_data where docid = NEW.docid), to_tsvector(coalesce(NEW.title,'''') ||'' ''|| coalesce(NEW.description,'''') ||'' ''|| coalesce(NEW.filename,'''') ||'' ''|| coalesce(NEW.filetype,'''') ||'' ''|| coalesce(NEW.data_words,'''')));
		ELSIF TG_OP = ''UPDATE'' THEN
			DELETE FROM doc_data_idx WHERE docid = NEW.docid and version = NEW.version;
			DELETE FROM doc_data_words_idx WHERE docid = NEW.docid and version = NEW.version;
			INSERT INTO doc_data_idx (docid, version, group_id, vectors) VALUES (NEW.docid, NEW.version, (select group_id from doc_data where docid = NEW.docid), to_tsvector(coalesce(NEW.title,'''') ||'' ''|| coalesce(NEW.description,'''')));
			INSERT INTO doc_data_words_idx (docid, version, group_id, vectors) VALUES (NEW.docid, NEW.version, (select group_id from doc_data where docid = NEW.docid), to_tsvector(coalesce(NEW.title,'''') ||'' ''|| coalesce(NEW.description,'''') ||'' ''|| coalesce(NEW.filename,'''') ||'' ''|| coalesce(NEW.filetype,'''') ||'' ''|| coalesce(NEW.data_words,'''')));
		ELSIF TG_OP = ''DELETE'' THEN
			DELETE FROM doc_data_idx WHERE docid = OLD.docid and version = OLD.version;
			DELETE FROM doc_data_words_idx WHERE docid = OLD.docid and version = OLD.version;
		END IF;
	-- **** forum table ****
