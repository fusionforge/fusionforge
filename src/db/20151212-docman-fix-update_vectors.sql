...
BEGIN
	...
	ELSIF table_name = ''doc_data'' THEN
		IF TG_OP = ''INSERT'' THEN
			...
			INSERT INTO doc_data_words_idx (docid, group_id, vectors) VALUES (NEW.docid, NEW.group_id, to_tsvector(coalesce(NEW.title,'''') ||'' ''|| coalesce(NEW.description,'''') ||'' ''|| coalesce(NEW.filename,'''') ||'' ''|| coalesce(NEW.filetype,'''') ||'' ''|| coalesce(NEW.data_words,'''')));
		ELSIF TG_OP = ''UPDATE'' THEN
			...
			UPDATE doc_data_words_idx SET group_id = NEW.group_id, vectors=to_tsvector(coalesce(NEW.title,'''') ||'' ''|| coalesce(NEW.description,'''') ||'' ''|| coalesce(NEW.filename,'''') ||'' ''|| coalesce(NEW.filetype,'''') ||'' ''|| coalesce(NEW.data_words,'''')) WHERE docid=NEW.docid;
