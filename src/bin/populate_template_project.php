...
if (isset($argv[1]) && in_array($argv[1], array('-h', '-?', '--help'))) {
	...
	db_begin();
	$project = new Group();
	$desc = sprintf(_('Template project %s (%s) populated on %s'), $argv[2], $argv[3], date("r"));
	if (!$project->create(session_get_user(), $argv[3], $argv[2], $desc, $desc)) {
		db_rollback();
		echo _('Error')._(': ')._('could not create group')._(': ').$project->getErrorMessage()."\n";
		exit(1);
	}
	if (!$project->setAsTemplate(true)) {
		db_rollback();
		echo _('Error')._(': ')._('could not mark group as template')._(': ').db_error()."\n";
		exit(1);
	}
	if (!populateProject($project)) {
		db_rollback();
...
printf(_('Group #%d %s (%s) populated successfully.')."\n", $project->getID(), $project->getUnixName(), $project->getPublicName());
exit(0);
