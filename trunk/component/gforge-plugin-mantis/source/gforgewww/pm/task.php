 ...
 * http://gforge.org/
 *
 * @version   $Id: task.php,v 1.24 2005/07/11 18:13:12 tperdue Exp $
 */
/*
...
switch ($func) {
	...
	case 'downloadcsv': {
		...
		if ($pg->userIsAdmin()) {
			include 'www/pm/downloadcsv.php';
			exit;
		} else {
			exit_permission_denied();
		}
	...
	case 'postuploadcsv': {
		...
		if ($pg->userIsAdmin()) {
			include 'www/pm/postuploadcsv.php';
		} else {
			exit_permission_denied();
		}
	...
	case 'detailtask' : {
		...
		if (session_loggedin() && $pg->userIsAdmin()) {
			include 'www/pm/mod_task.php';
		} else {
			include 'www/pm/detail_task.php';
		}
		break;
	}
