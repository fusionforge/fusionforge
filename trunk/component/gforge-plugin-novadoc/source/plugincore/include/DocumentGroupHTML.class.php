 ...
 * http://gforge.org/
 *
 * @version   $Id: DocumentGroupHTML.class.php,v 1.4 2006/10/27 17:42:55 pascal Exp $
 */
	...
	function buildArrays(&$group_arr, &$id_array, &$text_array, &$dont_display, $parent=0, $level=0,$levelMax=-1) {
		...
		for ($i = 0; $i < $child_count; $i++) {
    	    ...
    	    if( $levelMax==-1 || $level > $levelMax ){
	    		$this->buildArrays($group_arr, $id_array, $text_array, $dont_display, $doc_group->getID(), $level+1,$levelMax);
	    	}
		}
